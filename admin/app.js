...
function optionEditorHTML(p){const c=parseProductConfig(p);return `<div class="variant-box"><label><input type="checkbox" id="variantEnabled" ${c.enabled?'checked':''}> Product has options (Color, Size, Phone Model, etc.)</label><div id="variantFields"></div><input type="hidden" name="optionConfig" id="optionConfig"><input type="hidden" name="variants" id="variants" value='${esc(p?.variants||"[]")}'> </div>`}
function renderVariantFields(p){const c=parseProductConfig(p),wrap=$("variantFields");if(!wrap)return;const names=["Color","Size","Phone Model","Storage","Other"];wrap.innerHTML=(c.enabled?`<p class="muted">Add each option as comma-separated values.</p>`+names.map(n=>`<label>${n}<input data-varname="${n}" value="${esc((c.options||{})[n]?.join(', ')||'')}" placeholder="${n==='Size'?'40, 41, 42':n==='Color'?'Black, White, Red':n==='Phone Model'?'iPhone 13, Samsung A15':'Option 1, Option 2'}"></label>`).join(''):"");wrap.querySelectorAll('input[data-varname]').forEach(x=>x.addEventListener('input',syncVariants))}
