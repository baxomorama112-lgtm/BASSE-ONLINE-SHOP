...
CREATE TABLE IF NOT EXISTS orders(id TEXT PRIMARY KEY,product_id INTEGER,product_name TEXT,quantity INTEGER,customer_name TEXT,whatsapp TEXT,location TEXT,total INTEGER,payment_status TEXT DEFAULT 'PENDING',order_status TEXT DEFAULT 'NEW',waychit_request_id TEXT,created_at TEXT DEFAULT CURRENT_TIMESTAMP);`);
...
app.post("/api/admin/login",(req,res)=>{if(req.body.email===process.env.ADMIN_EMAIL&&req.body.password===process.env.ADMIN_PASSWORD){let t=crypto.randomBytes(32).toString("hex");sessions.set(t,Date.now()+432e5);res.json({token:t})}else res.status(401).json({error:"Invalid admin login"})});
app.get("/api/admin/products",guard,(req,res)=>res.json(db.prepare("SELECT * FROM products ORDER BY id DESC").all()));
app.post("/api/admin/products",guard,upload.single("image"),(req,res)=>{let b=req.body,img=req.file?"/uploads/"+req.file.filename:b.imageUrl||"";let x=db.prepare("INSERT INTO products(name,category,price,stock,description,image) VALUES(?,?,?,?,?,?)").run(b.name,b.category,+b.price,+b.stock||0,b.description||"",img);res.json(db.prepare("SELECT * FROM products WHERE id=?").get(x.lastInsertRowid))});
app.put("/api/admin/products/:id",guard,upload.single("image"),(req,res)=>{let p=db.prepare("SELECT * FROM products WHERE id=?").get(req.params.id);if(!p)return res.status(404).json({error:"Product not found"});let b=req.body,img=req.file?"/uploads/"+req.file.filename:(b.imageUrl||p.image);db.prepare("UPDATE products SET name=?,category=?,price=?,stock=?,description=?,image=? WHERE id=?").run(b.name,b.category,+b.price,+b.stock,b.description||"",img,p.id);res.json(db.prepare("SELECT * FROM products WHERE id=?").get(p.id))});
