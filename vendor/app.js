let token=localStorage.getItem("basseVendorToken")||"";const $=x=>document.getElementById(x),money=n=>"D"+Number(n||0).toLocaleString();const H=()=>({Authorization:"Bearer "+token});async function api(u,o={}){let r=await fetch(u,{...o,credentials:"same-origin",headers:{...H(),...(o.headers||{})}}),d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||"Request failed");return d}async function login(){try{let d=await api("/api/vendor/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({whatsapp:$("phone").value,pin:$("pin").value})});token=d.token;localStorage.setItem("basseVendorToken",token);start()}catch(e){alert(e.message)}}function start(){$("login").classList.add("hidden");$("app").classList.remove("hidden");refresh()}function logout(){
  ...
  $("form").innerHTML=`<h2>Create Vendor Account</h2><p class="form-note">Apply to become a seller on BASSE ONLINE SHOP. Your account will be reviewed by Admin before you can log in.</p><form class="form" id="vf"><input name="fullName" placeholder="Owner name" autocomplete="name" required><input name="businessName" placeholder="Business / shop name" autocomplete="organization" required><input name="whatsapp" placeholder="WhatsApp number" inputmode="numeric" autocomplete="tel" required><select name="category" required><option value="">Business category</option><option>Fashion</option><option>Electronics</option><option>Phones</option><option>Beauty</option><option>Home & Living</option><option>Accessories</option><option>Food</option><option>Other</option></select><input name="location" placeholder="Shop location" autocomplete="address-level2" required><textarea name="description" placeholder="Short description of your business"></textarea><input name="password" type="password" inputmode="numeric" maxlength="5" minlength="4" pattern="\d{4,5}" placeholder="Create 4–5 digit PIN" required><input name="confirmPin" type="password" inputmode="numeric" maxlength="5" minlength="4" pattern="\d{4,5}" placeholder="Confirm PIN" required><div class="form-actions"><button type="button" class="secondary" onclick="closeModal()">CANCEL</button><button>SUBMIT APPLICATION</button></div></form>`;
  $("vf").onsubmit=submitVendorApplication;
 }
 async function submitVendorApplication(e){
  e.preventDefault();
  const f=new FormData(e.target), pin=String(f.get("password")||""), confirm=String(f.get("confirmPin")||"");
  if(pin!==confirm){alert("PINs do not match.");return}
  try{
   const payload={fullName:f.get("fullName"),businessName:f.get("businessName"),whatsapp:f.get("whatsapp"),category:f.get("category"),location:f.get("location"),description:f.get("description"),password:pin};
   ...
   $("form").innerHTML=`<div class="success-box"><div class="success-icon">✓</div><h2>Application Submitted</h2><p>Your vendor account has been created and is awaiting Admin approval.</p><p>Once approved, you can return here and log in with your WhatsApp number and PIN.</p><button onclick="closeModal()">DONE</button></div>`;
