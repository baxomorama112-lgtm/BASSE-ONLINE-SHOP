let token=localStorage.getItem("basseVendorToken")||"";const $=x=>document.getElementById(x),money=n=>"D"+Number(n||0).toLocaleString();const H=()=>({Authorization:"Bearer "+token});async function api(u,o={}){let r=await fetch(u,{...o,credentials:"same-origin",headers:{...H(),...(o.headers||{})}}),d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||"Request failed");return d}async function login(){try{let d=await api("/api/vendor/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({whatsapp:$("phone").value,pin:$("pin").value})});token=d.token;localStorage.setItem("basseVendorToken",token);start()}catch(e){alert(e.message)}}function start(){$("login").classList.add("hidden");$("app").classList.remove("hidden");refresh()}function logout(){
  ...
  $("form").innerHTML=`<h2>Create Vendor Account</h2><p class="form-note">Apply to become a seller on BASSE ONLINE SHOP. Verify your email first, then Admin can approve your account.</p><form class="form" id="vf"><input name="fullName" placeholder="Owner name" autocomplete="name" required><input name="businessName" placeholder="Business / shop name" autocomplete="organization" required><input name="whatsapp" placeholder="WhatsApp number" inputmode="numeric" autocomplete="tel" required><input name="email" type="email" placeholder="Gmail / Email address" autocomplete="email" required><select name="category" required><option value="">Business category</option><option>Fashion</option><option>Electronics</option><option>Phones</option><option>Beauty</option><option>Home & Living</option><option>Accessories</option><option>Food</option><option>Other</option></select><input name="location" placeholder="Shop location" autocomplete="address-level2" required><textarea name="description" placeholder="Short description of your business"></textarea><input name="password" type="password" inputmode="numeric" maxlength="5" minlength="4" pattern="[0-9]{4,5}" autocomplete="new-password" placeholder="Create 4–5 digit PIN" required><input name="confirmPin" type="password" inputmode="numeric" maxlength="5" minlength="4" pattern="[0-9]{4,5}" autocomplete="new-password" placeholder="Confirm PIN" required><div class="form-actions"><button type="button" class="secondary" onclick="closeModal()">CANCEL</button><button>SUBMIT APPLICATION</button></div></form>`;
 ...
 function showVendorOtp(email){
   ...
   $("verifyVendorOtp").onclick=()=>verifyVendorOtp(email);
   $("resendVendorOtp").onclick=()=>resendVendorOtp(email);
 }
 async function verifyVendorOtp(email){
   const code=$("vendorOtp").value.trim(); if(!/^\d{6}$/.test(code)){alert("Enter the 6-digit code.");return}
   try{const r=await fetch("/api/vendors/verify-email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,code})}),d=await r.json();if(!r.ok)throw Error(d.error||"Verification failed");$("form").innerHTML=`<div class="success-box"><div class="success-icon">✓</div><h2>Email Verified</h2><p>Your vendor account has been created and your PIN has been saved securely.</p><p>Your application is now waiting for Admin approval.</p><button onclick="closeModal()">DONE</button></div>`;}catch(e){alert(e.message)}
 }
 async function resendVendorOtp(email){try{const r=await fetch("/api/vendors/resend-otp",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email})}),d=await r.json();if(!r.ok)throw Error(d.error||"Could not resend code");alert("A new verification code was sent.");}catch(e){alert(e.message)}}
