...
function chatInbox(){try{const x=JSON.parse(localStorage.getItem("basseChatInbox")||"[]");return Array.isArray(x)?x:[]}catch{return []}}
function saveChatInbox(list){try{localStorage.setItem("basseChatInbox",JSON.stringify(list.slice(0,30)))}catch{}}
function rememberChatConversation(c){if(!c?.id)return;const old=chatInbox().find(x=>Number(x.id)===Number(c.id));const list=chatInbox().filter(x=>Number(x.id)!==Number(c.id));list.unshift({id:Number(c.id),vendorId:c.vendor_id?Number(c.vendor_id):null,vendorName:c.vendor_id?(c.vendor_name||"Vendor"):"Admin",updatedAt:Date.now(),lastMessage:c.last_message||"",unread:old?.unread||0});saveChatInbox(list);}
function setChatUnreadBadge(n){const b=$("chatUnreadBadge");if(!b)return;n=Math.max(0,Number(n)||0);b.textContent=n>99?"99+":String(n);b.classList.toggle("hidden",n===0);}
async function refreshChatUnread(){const list=chatInbox();if(!list.length){setChatUnreadBadge(0);return;}let total=0;const rows=[];for(const x of list){try{const d=await chatApi('/api/chat/messages/'+x.id);const unread=(d.messages||[]).filter(m=>m.sender_type!=="customer"&&!m.read_at).length;total+=unread;rows.push({...x,unread});}catch{rows.push(x)}}saveChatInbox(rows);setChatUnreadBadge(total);}
function startChatUnreadPolling(){refreshChatUnread();if(window._chatUnreadTimer)clearInterval(window._chatUnreadTimer);window._chatUnreadTimer=setInterval(refreshChatUnread,5000);}
...
async function openChat(vendorId=null,vendorName="Admin",existingId=null){
  stopChatPoll();
  const modal=$("modal"), who=customerIdentity();
  modal.innerHTML=`<div class="sheet chat-sheet"><button class="close" onclick="openChatInbox()">×</button><div class="chat-head-actions"><button class="chat-inbox-btn" type="button" onclick="openChatInbox()">← Inbox</button></div><div class="checkout-head"><span class="support-modal-icon">💬</span><div><small>BASSE ONLINE SHOP</small><h2>Chat with ${esc(vendorName)} <span class="chat-online">● ONLINE</span></h2></div></div><div id="chatMessages" class="chat-messages"><div class="muted">Loading conversation…</div></div><div class="chat-compose"><input id="chatText" placeholder="Type your message…" autocomplete="off"><button class="pay chat-send-btn" onclick="sendChatMessage()">SEND <span>→</span></button></div></div>`;
  modal.classList.add('show');
  try{
    let c=null;
    if(existingId){try{const q=await chatApi('/api/chat/messages/'+existingId);c=q.conversation}catch{}}
    if(!c){const d=await chatApi('/api/chat/conversations',{method:'POST',body:JSON.stringify({customerId:who.customerId,customerName:who.name,customerWhatsapp:who.whatsapp,vendorId:vendorId||null})});c=d.conversation;}
    activeChatId=c.id;rememberChatConversation({...c,vendor_name:vendorName});await loadChatMessages();try{await chatApi("/api/chat/messages/"+activeChatId+"/read",{method:"POST",body:JSON.stringify({customerWhatsapp:who.whatsapp})});}catch{}refreshChatUnread();chatPoll=setInterval(loadChatMessages,3000);setTimeout(()=>$("chatText")?.focus(),100);
  }catch(e){$("chatMessages").innerHTML=`<p class="muted">${esc(e.message)}</p>`}
}
async function openChatInbox(){
  stopChatPoll();
  const modal=$("modal");
  modal.innerHTML=`<div class="sheet chat-inbox-sheet messenger-home-sheet"><button class="close" onclick="closeModal()">×</button><div class="chat-messenger-head"><div class="chat-welcome-icon">💬</div><div><div class="chat-welcome-kicker">BASSE ONLINE SHOP</div><h2>Messenger</h2><p>Chat with BASSE Admin or any approved Vendor.</p></div></div><div class="chat-start-grid"><button type="button" class="chat-start-card" onclick="openChat(null,'Admin')"><span>👨🏽‍💼</span><b>Chat with Admin</b><small>Orders, payments, delivery & support</small></button><button type="button" class="chat-start-card" onclick="openVendorChatPicker()"><span>🏪</span><b>Chat with a Vendor</b><small>Ask about products, prices & availability</small></button></div><div id="chatVendorPicker" class="chat-vendor-picker hidden"></div><div class="chat-inbox-heading"><div><b>Messages</b><small>Your conversations are saved for up to 72 hours.</small></div></div><div id="chatInboxList" class="chat-inbox-list"></div></div>`;
  modal.classList.add('show');
  await renderChatInbox();
}
async function openVendorChatPicker(){
  const box=$("chatVendorPicker"); if(!box)return;
  box.classList.remove('hidden'); box.innerHTML='<div class="chat-vendor-picker-title"><b>Choose a Vendor</b><button type="button" onclick="this.parentElement.parentElement.classList.add(\'hidden\')">×</button></div><div class="chat-vendor-picker-list">Loading vendors…</div>';
  try{
    const vendors=await chatApi('/api/stores');
    const list=Array.isArray(vendors)?vendors:[];
    box.querySelector('.chat-vendor-picker-list').innerHTML=list.length?list.map(v=>`<button type="button" class="chat-vendor-choice" onclick="openChat(${Number(v.id)},'${esc(v.business_name||'Vendor').replace(/'/g,"\\'")}')"><span class="chat-avatar">V</span><span><b>${esc(v.business_name||'Vendor')}</b><small>${esc(v.category||'Vendor')} · ${Number(v.product_count||0)} products</small></span><strong>›</strong></button>`).join(''):'<div class="chat-inbox-empty">No approved vendors are available yet.</div>';
  }catch(e){box.querySelector('.chat-vendor-picker-list').innerHTML=`<div class="chat-inbox-empty">${esc(e.message||'Could not load vendors.')}</div>`}
}
async function renderChatInbox(){const box=$("chatInboxList");if(!box)return;const list=chatInbox();if(!list.length){box.innerHTML='<div class="chat-inbox-empty">No conversations yet.<br><small>Start a chat with Admin or a Vendor.</small></div>';return;}const rows=[];for(const x of list){try{const d=await chatApi('/api/chat/messages/'+x.id);const last=d.messages?.[d.messages.length-1];const name=d.conversation?.vendor_id?(x.vendorName||'Vendor'):'Admin';rows.push({...x,vendorName:name,lastMessage:last?.message||x.lastMessage||'No messages',updatedAt:last?new Date(last.created_at).getTime():(x.updatedAt||0)});rememberChatConversation({...d.conversation,vendor_name:name,last_message:last?.message||''});}catch{rows.push(x)}}rows.sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));saveChatInbox(rows);box.innerHTML=rows.map(x=>`<button class="chat-inbox-row ${x.unread?'chat-row-unread':''}" type="button" onclick="openChat(${x.vendorId||'null'},'${esc(x.vendorName||'Admin').replace(/'/g,"\'")}',${Number(x.id)})"><span class="chat-avatar">${x.vendorId?'V':'A'}</span><span class="chat-inbox-copy"><b>${esc(x.vendorName||'Admin')}</b><small>${esc(x.lastMessage||'No messages')}</small></span>${x.unread?`<span class="chat-row-badge">${x.unread>99?'99+':x.unread}</span>`:''}<span class="chat-chevron">›</span></button>`).join('');setChatUnreadBadge(rows.reduce((n,x)=>n+(Number(x.unread)||0),0));}
async function loadChatMessages(){if(!activeChatId)return;try{const d=await chatApi('/api/chat/messages/'+activeChatId);const box=$("chatMessages");if(!box)return;box.innerHTML=d.messages.length?d.messages.map(m=>`<div class="chat-bubble ${m.sender_type==='customer'?'mine':'theirs'}"><div>${esc(m.message)}</div>${m.product_id?`<button class="chat-product" onclick="buy(${m.product_id})">VIEW PRODUCT →</button>`:''}<small>${new Date(m.created_at).toLocaleString()}</small></div>`).join(''):`<div class="muted chat-empty">Start the conversation.</div>`;box.scrollTop=box.scrollHeight;const last=d.messages?.[d.messages.length-1];const c=d.conversation||{};rememberChatConversation({...c,vendor_name:c.vendor_id?'Vendor':'Admin',last_message:last?.message||''})}catch{}}
function quickChat(text){const input=$("chatText");if(!input)return;input.value=text;input.style.fontSize='16px';input.focus();}
