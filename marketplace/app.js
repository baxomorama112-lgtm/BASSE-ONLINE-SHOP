...
function rememberChatConversation(c){if(!c?.id)return;const list=chatInbox().filter(x=>Number(x.id)!==Number(c.id));list.unshift({id:Number(c.id),vendorId:c.vendor_id?Number(c.vendor_id):null,vendorName:c.vendor_id?(c.vendor_name||"Vendor"):"Admin",updatedAt:Date.now(),lastMessage:c.last_message||""});saveChatInbox(list);}
...
async function openChat(vendorId=null,vendorName="Admin",existingId=null){
  ...
  modal.innerHTML=`<div class="sheet chat-sheet"><button class="close" onclick="openChatInbox()">×</button><div class="chat-head-actions"><button class="chat-inbox-btn" type="button" onclick="openChatInbox()">← Inbox</button></div><div class="checkout-head"><span class="support-modal-icon">💬</span><div><small>BASSE ONLINE SHOP</small><h2>Chat with ${esc(vendorName)} <span class="chat-online">● ONLINE</span></h2></div></div><div class="chat-quick-actions"><button type="button" onclick="quickChat('Is this product available?')">Ask about product</button><button type="button" onclick="quickChat('Can you help me with my order?')">My order</button><button type="button" onclick="quickChat('I need help with payment.')">Payment help</button><button type="button" onclick="quickChat('Can you help with delivery?')">Delivery help</button></div><div id="chatMessages" class="chat-messages"><div class="muted">Loading conversation…</div></div><div class="chat-compose"><input id="chatText" placeholder="Type your message…" autocomplete="off"><button class="pay chat-send-btn" onclick="sendChatMessage()">SEND <span>→</span></button></div></div>`;
  modal.classList.add('show');
  try{
    let c=null;
    ...
    activeChatId=c.id;rememberChatConversation({...c,vendor_name:vendorName});await loadChatMessages();chatPoll=setInterval(loadChatMessages,3000);setTimeout(()=>$("chatText")?.focus(),100);
...
async function renderChatInbox(){const box=$("chatInboxList");if(!box)return;const list=chatInbox();if(!list.length){box.innerHTML='<div class="chat-inbox-empty">No conversations yet.<br><small>Start a chat with Admin or a Vendor.</small></div>';return;}const rows=[];for(const x of list){try{const d=await chatApi('/api/chat/messages/'+x.id);const last=d.messages?.[d.messages.length-1];const name=d.conversation?.vendor_id?(x.vendorName||'Vendor'):'Admin';rows.push({...x,vendorName:name,lastMessage:last?.message||x.lastMessage||'No messages',updatedAt:last?new Date(last.created_at).getTime():(x.updatedAt||0)});rememberChatConversation({...d.conversation,vendor_name:name,last_message:last?.message||''});}catch{rows.push(x)}}rows.sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));saveChatInbox(rows);box.innerHTML=rows.map(x=>`<button class="chat-inbox-row" type="button" onclick="openChat(${x.vendorId||'null'},'${esc(x.vendorName||'Admin').replace(/'/g,"\'")}',${Number(x.id)})"><span class="chat-avatar">${x.vendorId?'V':'A'}</span><span class="chat-inbox-copy"><b>${esc(x.vendorName||'Admin')}</b><small>${esc(x.lastMessage||'No messages')}</small></span><span class="chat-chevron">›</span></button>`).join('');}
