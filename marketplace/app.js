...
async function openChatInbox(){
  stopChatPoll();const modal=$("modal");modal.innerHTML=`<div class="sheet chat-inbox-sheet"><button class="close" onclick="closeModal()">×</button><div class="chat-welcome"><div class="chat-welcome-icon">💬</div><div class="chat-welcome-kicker">BASSE ONLINE SHOP</div><h2>How can we help you today?</h2><p>Chat with Admin or a Vendor about products, prices, orders, payments and delivery.</p><button class="pay chat-lets-btn" onclick="openChat(null,'Admin')">LET'S CHAT <span>→</span></button></div><div class="chat-inbox-heading"><div><b>Messages</b><small>Your conversations are saved here.</small></div></div><div id="chatInboxList" class="chat-inbox-list"></div></div>`;modal.classList.add('show');await renderChatInbox();}
...
function quickChat(text){const input=$("chatText");if(!input)return;input.value=text;input.focus();}
